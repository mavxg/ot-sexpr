...
AttributedString.prototype.type = "string";
AttributedString.prototype.nodeAt = function(point, parent) {
	if (point === 0) return {node:this, offset:0, index:-1, parent:parent, type:"string"};
	if (point >= this.size) throw "Point outside of string";
	return {node:this, offset:point, index:(point-1), parent:parent, type:"string"};
};
AttributedString.prototype.nodesAt = function(point, _start, _ret) {
	var ret = _ret || [];
	var start = _start || 0;
	ret.push({node:this, point:start, type:"string"});
	if (point >= this.size) throw "Point outside of string";
	return ret;
};
...
AttributedString.prototype.attributesAt = function(point, ret) {
	ret = ret || [];
	var atts = this.attributes || [[0,{}]];
	if (atts.length === 1) {
		ret.push({node:this, type:"string", attributes:atts[0][1]});
		return ret;
	}
	var _remainder = point - 1; //subtract 1 for the string start
	for (var i = 0; i < atts.length && _remainder >= 0; i++) {
		var a = atts[i];
		var l = a[0];
		if (l >= _remainder) {
			ret.push({node:this, type:"string", attributes:a[1]});
			return ret;
		}
		_remainder -= l;
	};
	ret.push({node:this, type:"string", attributes:{}});
	return ret;
};
