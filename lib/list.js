...
List.prototype.push = function(v) {
	if (v instanceof List || v instanceof AttributedString)
		v.parent = this;
	this.values.push(v);
	this.length++;
	this.size += (v.size || 1);
...
List.prototype.nextSibling = function() {
	if (!this.parent instanceof List) return;
	var i = this.parent.values.indexOf(this);
	return this.parent.index(i+1);
};
