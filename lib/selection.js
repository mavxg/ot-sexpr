...
Selection.prototype.slice = function(start, end) {
  ...
  for (var i = 0; i < this.regions.length; i++) {
    ...
    ret.push(new Region(
      Math.min(l, Math.max(-1,region.focus - start)),
      Math.min(l, Math.max(-1,region.anchor - start))));
  }
...
Selection.prototype.equals = function(other) {
  if (other.regions.length !== this.regions.length) return false;
  var t = this.regions;
  var o = other.regions;
  for (var i = t.length - 1; i >= 0; i--) {
    if (!t[i].equals(o[i])) return false;
  };
  return true;
};
