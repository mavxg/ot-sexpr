...
function apply(snapshot, ops) {
  return (typeof(snapshot) === 'string') ?
    serialize(applyDeserialized(deserialize(snapshot), ops)) :
    applyDeserialized(snapshot, ops);
}

function applyDeserialized(d,ops) {
  //console.log('Apply to doc size: ' + d.size)
  //stack and t(op) for traversing output document
  var stack = []
  var t = [];
  t.toSexpr = function() { return "[]"; };
  var op;
  ...
  function process(op) {
    switch (op.type) {
      ...
      case PUSH:
        stack.push(t);
        //console.log('Pushed (PUSH): ' + (t.toSexpr ? t.toSexpr() : JSON.stringify(t)));
        if (op.value === LIST) {
      ...
      case POP:
        var tmp = stack.pop();
        //console.log('Popped (POP): ' + (tmp.toSexpr ? tmp.toSexpr() : JSON.stringify(tmp)));
        tmp.push(t);
        t = tmp;
        if (op.value === STRING)
          isString = false;
  ...
  function retain(length, attributes, unattributes) {
    ...
    while (length > 0) {
      while (length > 0 && index >= dp.length) {
        //POP output doc
        var tmp = stack.pop();
        //console.log('Popped (retain '+ length +'): ' + (tmp && tmp.toSexpr ? tmp.toSexpr() : JSON.stringify(tmp)));
        tmp.push(t);
        t = tmp;
        //POP input doc
        var pp = docStack.pop();
      ...
      if (isString) {
        ...
        if (c instanceof List) {
          if (c.size <= length) {
            ...
            stack.push(t);
            //console.log('Pushed (retain '+ length +' list): ' + (t.toSexpr ? t.toSexpr() : JSON.stringify(t)));
            t = n;
            //go deeper into the document
            docStack.push({index:index, dp:dp});
            dp = c;
          ...
          if (c.size <= length) {
            ...
            //go deeper into the output stack
            stack.push(t);
            //console.log('Pushed (retain '+ length +' string): ' + (t.toSexpr ? t.toSexpr() : JSON.stringify(t)));
            t = n;
            //go deeper into the document
            docStack.push({index:index, dp:dp});
            dp = c;
  ...
  if (offset < d.size) {
    //console.log('Retain to end: ' + d.size + ' from: ' + offset);
    retain(d.size - offset);
  }
