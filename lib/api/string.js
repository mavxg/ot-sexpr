...
AttributedString.prototype.clearText = function(start, end, ret) {
	var _start = (start === undefined || start < 0 ? 0 : start);
	var _end = (end === undefined || end > this.size ? this.size : end);
	var offset = 1;
	ret = ret || [];
	push(ret, ops.retain(_start+1)); //retain to start
	var s = Math.max(0,_start-1)
	var e = Math.max(0,Math.min(this.str.length, _end-2))
	push(ret, ops.delete(this.str.slice(s, e),'char'))
	push(ret, ops.retain(this.size - _end + 1)); //retain to end
	return ret;
};
