...
AttributedString.prototype._reattributeText = function(start, end, attributes, unattributes, ret) {
	...
	for (var i = 0; i < chunks.length && offset < _end; i++) {
		var chunk = chunks[i];
		...
		if (offset < _end && offset + n > _start) {
			...
			if (offset + n > _end) {
				push(ret, ops.retain(_end - offset, aa, ua));
				//don't need to retain here because we have reached the end of where we need to attribute
			} else {
				push(ret, ops.retain(n, aa, ua));
			}
		} else {
