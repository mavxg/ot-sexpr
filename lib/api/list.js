...
List.prototype.offset = function() {
	...
	var parents_offset = this.parent.offset();
	if (parents_offset === -1) return -1;

	var offset_in_parent = this.parent.offsetOf(this);
	if (offset_in_parent === -1) return -1;

	return parents_offset + offset_in_parent;
};
List.prototype.offsetOf = function(child) {
	var offset = 1;
	for (var i = 0; i < this.values.length; i++) {
		var c = this.values[i];
		if (c === child) return offset;
		offset += (c instanceof List || c instanceof AttributedString) ?
			c.size : 1;
	};
	return -1;
};
